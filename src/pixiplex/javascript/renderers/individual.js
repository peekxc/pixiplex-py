...
class IndividualNodeRenderer {
  ...
  init(nodes, links) {
    ...
    this._apply_node_styles(this.node_styles);
  ...
  set_node_styles(styles) {
    this._apply_node_styles(styles);
  }
 ...
 */
export const enable_individual_renderer = (pixiplex) => {
  const renderer = new IndividualNodeRenderer(pixiplex);
  return install_renderer_runtime(pixiplex, renderer, "__individual_renderer");
};
...
class PixiplexIndividual extends Pixiplex {
  ...
  constructor(nodes = [], links = [], width = 250, height = 250, scale = 2.0, forces = {}) {
    ...
    enable_individual_renderer(this);
  }
}
