 ...
 * @returns {void}
 */
const write_quad_uvs = (uvs, quadIndex) => {
  const o = quadIndex * 8;
  uvs[o] = 0;
  uvs[o + 1] = 0;
  uvs[o + 2] = 1;
 ...
 * @returns {void}
 */
const write_quad_indices = (indices, quadIndex) => {
  const v = quadIndex * 4;
  const o = quadIndex * 6;
  indices[o] = v;
  indices[o + 1] = v + 1;
...
class MeshPrimitiveRenderer {
   ...
   * @returns {void}
   */
  _ensure_edge_capacity(linkCount) {
    ...
    for (let i = 0; i < linkCount; i += 1) {
      write_quad_uvs(this.edge_uvs, i);
      write_quad_indices(this.edge_indices, i);
    }
   ...
   * @returns {void}
   */
  _ensure_node_capacity(nodeCount) {
    ...
    for (let i = 0; i < nodeCount; i += 1) {
      write_quad_uvs(this.node_uvs, i);
      write_quad_indices(this.node_indices, i);
    }
  ...
  init(nodes, links) {
    ...
    this._ensure_edge_capacity(links.length);
    this._ensure_node_capacity(this.sim_nodes.length);
    ...
    this._update_edge_vertices();
    this._update_node_vertices();
  }
 ...
 */
export const enable_mesh_renderer = (pixiplex) => {
  const renderer = new MeshPrimitiveRenderer(pixiplex);
  return install_renderer_runtime(pixiplex, renderer, "__mesh_primitive_renderer");
};
