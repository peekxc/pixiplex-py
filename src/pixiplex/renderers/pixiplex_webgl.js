 ...
 * @returns {void}
 */
const setUniformHexColor = (gl, uniformLocation, hex, alpha = 1) => {
...
class WebGLPrimitiveRenderer {
  ...
  _ensureCanvas() {
    ...
    this.lineProgram = createProgram(this.gl, LINE_VS, LINE_FS);
    this.pointProgram = createProgram(this.gl, POINT_VS, POINT_FS);
   ...
   * @returns {void}
   */
  _ensureCapacity(numLinks, numNodes) {
  ...
  draw() {
    ...
    this._ensureCapacity(links.length, nodes.length);
    ...
    setUniformHexColor(gl, this.lineColorLoc, this.pixiplex.line_style?.color, this.pixiplex.line_style?.alpha ?? 1);
    ...
    setUniformHexColor(gl, this.pointColorLoc, nodeStyle.color ?? NODE_STYLE.color, nodeStyle.alpha ?? NODE_STYLE.alpha);
 ...
 */
export const enableWebGLPrimitiveRenderer = (pixiplex) => {
  const renderer = new WebGLPrimitiveRenderer(pixiplex);
  return installRendererRuntime(pixiplex, renderer, "__webgl_primitive_renderer");
};
...
class PixiplexWebGL extends Pixiplex {
  ...
  constructor(nodes = [], links = [], width = 250, height = 250, scale = 2.0, forces = {}) {
    ...
    enableWebGLPrimitiveRenderer(this);
  }
}
